...
{
	/// <summary> General settings block of the Object Placer tool: tool activation, interaction mode, raycast layer filter and auto-parent.
	...
	public class ObjectPlacerGeneralSection : VisualElement
	{
		...
		public ObjectPlacerMode Mode => (ObjectPlacerMode)modeField.value;
		...
		public ObjectPlacerGeneralSection()
		{
			...
			modeField = new EnumField("Mode", ObjectPlacerToolState.Mode);
			modeField.RegisterValueChangedCallback(ev => ObjectPlacerToolState.SetMode((ObjectPlacerMode)ev.newValue));
			...
			dragScatterSpacingField.style.display = GetDisplay(IsDragThrottled(ObjectPlacerToolState.Mode));
		...
		{
			// Resync in case state changed elsewhere (e.g. the Scene view overlay) while this section's
			// panel was detached - a backgrounded window tab stops receiving state change events.
		...
		void OnStateModeChanged(ObjectPlacerMode mode)
		{
			modeField.SetValueWithoutNotify(mode);
			dragScatterSpacingField.style.display = GetDisplay(IsDragThrottled(mode));
		}

		static bool IsDragThrottled(ObjectPlacerMode mode) => mode is ObjectPlacerMode.DragScatter or ObjectPlacerMode.Erase;
